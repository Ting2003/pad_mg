...
double Circuit::SA(double Frozen_T){	
	...
	//double cur_penalty;
	//for(size_t i=0;i<nodelist.size()-1;i++)
	//	prev_penalty += penalty(VDD-nodelist[i]->value, max_IRdrop);
	//double cur_sumIRdrop = 0;
	...
	int count_break=0;
	int temp_iter = 0;
	while (T > Frozen_T){// && temp_iter<10){
		temp_iter++;
		Move_num_rejected = 0;  
		for (iter_move=1; iter_move<=Movement; iter_move++){
			//clog<<"prev_maxIR: "<<prev_maxIRdrop<<endl;
			// compute one movement
			one_move(nodesUpdate_move, rm_pad, 
				add_pad, rm_index, iter_move,
				rm_net, add_net, index_rm_net);
			...
			//cur_penalty = 0;
			//for(size_t i=0;i<nodelist.size()-1;i++)
			//	cur_penalty += penalty(VDD-nodelist[i]->value, prev_maxIRdrop);	
			//clog<<"prev, new max: "<<prev_maxIRdrop<<" "<<max_IRdrop<<endl;
			double change_max = max_IRdrop - prev_maxIRdrop;
			//double change_cost = cur_sumIRdrop - 
			//	prev_sumIRdrop;
			//double change_cost = cur_penalty - prev_penalty;
			
			//clog<<endl<<"cur_penalty, prev_penalty, change_cost: "<<cur_penalty<<" "<<prev_penalty<<" "<<change_cost<<endl;
			//if(T>1){
				change_cost= update_cost(
				nodesUpdate_move, iter_T, 
				change_cost_total, old_voltages);
			/*}else
				change_cost = max_IRdrop-prev_maxIRdrop;*/
			//clog<<"change cost: "<<change_cost<<endl;
			//if(change_max <0){
			if(change_cost<0){// && change_max < 0){
				//clog<<"accept. "<<endl;
				//if(change_cost <0)
				//if(change_cost_total < 0)
				//	prev_sumIRdrop = cur_sumIRdrop;
				accept_move(nodesUpdate_move, 
				  old_voltages, rm_index, add_pad,
				  rm_net);
				//clog<<"prev_maxIRdrop, max_IRdrop: "<<prev_maxIRdrop<<" "<<max_IRdrop<<endl;
				prev_maxIRdrop = max_IRdrop;
				//prev_penalty = cur_penalty;
				//clog<<"new_maxIRdrop: "<<prev_maxIRdrop<<endl;
			}
			else{
				//if(T>1){
...
void Circuit::solve_coarse(double Frozen_T){
	...
	locate_maxIRdrop();
	t2 = clock();
	double time = 1.0*(t2-t1)/CLOCKS_PER_SEC;
	clog<<"time for SA is: "<<time<<endl;
...
//	clog<<"initial max_IRdrop is: 	"<<max_IRdrop<<endl;
	...
	*/
	...
	bool flag=false;
	SA_new(Frozen_T, flag);
	locate_maxIRdrop();
	clog<<"max_IRdrop after SA  :	"<<max_IRdrop<<endl;
	/*if(max_IRdrop>0.37)
	SA(Frozen_T);
	locate_maxIRdrop();
	clog<<"max_IRdrop after SA again: "<<max_IRdrop<<endl;	*/
}
...
inline double Circuit::penalty(double v, double vworst){
	...
	if(v>=0.8*vworst && v<0.98*vworst)
		...
		penalty = v*v*100;
	//else if(v>=0.5*vworst && v<0.8*vworst)
	//	penalty = v*v*10;
	else
		penalty = 0;
	return penalty;
...
void Circuit::one_move_new(vector<Node*>&nodesUpdate_move,
	Node *&rm_pad, Node *&add_pad,  
	size_t &rm_pad_index, size_t iter_move){
	size_t VDD_num = VDD_set.size();
	vector<Node*> nbr_pads;
...
void Circuit::update_pad_value(Node *rm_pad, Node *add_pad, 
	...
	while(max_diff > eps0){// && iter<LIMIT){
		...
		max_diff = 0;
		diff = 1;
		double vol_old = rm_pad->value;
		double vol_new = 0;
		size_t count = 2;
		while(q.isEmpty()==false && abs(diff)>eps0){//5e-9){
			...
			//cout<<"nd diff: "<<*nd<<" "<<abs(diff)<<endl;
			/*if(abs(diff)>max_diff) {
				max_diff =abs(diff);
				max_nd = nd;
			}*/
			if(nd->name == rm_pad->name)
			vol_new = rm_pad->value;
			max_diff = abs(vol_new - vol_old);
			}
		...
		cout<<"count in queue is: "<<count<<endl;
		//clog<<"iter, max_diff, nd: "<<iter<<" "<<max_diff<<endl;
			//" "<<*max_nd<<endl;
		iter++;
		//clog<<"max_node: "<<*max_nd<<endl;
	}
	t2= clock();
	cout<<"iter: "<<iter<<endl;
	//cout<<"iteration for moving one pad: "<<iter<<endl;
		//clog<<"iterationfor one updating: "<<temp_iter<<endl;
	cout<<"time for moving one pad: "<<1.0*(t2-t1)/CLOCKS_PER_SEC<<endl;
...
double Circuit::update_node_value(int iter, Node *&rm_pad, Node *&nd){
	...
	double radius_x = fabs(nd->pt.x-rm_pad->pt.x);
	double radius_y = fabs(nd->pt.y-rm_pad->pt.y);
	//cout<<"nd, rm: "<<*nd<<" "<<*rm_pad<<endl;
	//cout<<"Rx, Ry: "<<radius_x<<" "<<radius_y<<endl;
...
double Circuit::update_cost(vector<Node*> &nodesUpdate_move, 
	...
	double change_cost = 0;
	Node *nd;
	double cost_origin, cost_new;
	cost_origin = 0;
	cost_new = 0;
	for (size_t i=0; i<nodesUpdate_move.size();i++){
		nd = nodesUpdate_move[i] ;
		//cost_new += penalty(VDD-nd->value, max_IRdrop);
		//cost_origin += penalty(VDD-old_voltages[nd->rid],max_IRdrop);
		//change_cost += old_voltages[nd->rid]-nd->value;
	...
	//clog<<"cost_new, cost_origin: "<<cost_new<<" "<<cost_origin<<endl;
	return change_cost;
}
...
void Circuit::accept_move(vector<Node*>&nodesUpdate_move, 
	...
	VDD_set[rm_index]=add_pad;
	//locate_maxIRdrop();
	//clog<<"max_IRdrop after: "<<max_IRdrop<<endl;
	// free rm_net
	//if(rm_net != NULL){
		//free(rm_net);
		//rm_net = NULL;
...
void Circuit::accept_move_new(vector<Node*>&nodesUpdate_move, 
		double *old_voltages, size_t rm_index, 
		Node *add_pad){
	Node *nd;
	//size_t size = nodesUpdate_move.size()-1;
...
void Circuit::reject_move(vector<Node*>&nodesUpdate_move, 
	...
	// free add_net and restore rm_net
	//net_set[VOLTAGE][index_rm_net] = rm_net;
	//free(add_net);
}

void Circuit::reject_move_new(vector<Node*>&nodesUpdate_move, 
	Node *rm_pad, Node *add_pad, double *old_voltages){
	Node *nd;
	for (size_t i=0; i<nodelist.size()-1;i++){//nodesUpdate_move.size();i++){
		nd = nodelist[i];//nodesUpdate_move[i];
...
bool Circuit::acceptProb(double p){
	return rand()<((double)RAND_MAX+1.0)*p;
}
...
double Circuit::optimize_pad_assign_new(){
	...
	while(abs(max_IRdrop) < abs(old_maxIR)){
		...
		if(abs(max_IRdrop) > abs(old_maxIR)){
			reject_move(nodesUpdate_move, rm_pad, add_pad, 
				old_voltages, rm_net, add_net, 
				index_rm_net);
			break;
		}
		accept_move(nodesUpdate_move, old_voltages, min_index,
			add_pad, rm_net);	
		//clog<<"accept move. "<<endl;	
...
size_t Circuit::rebuild_voltage_nets_one_move(Node *rm_node, 
